...
export const BoardCard = ({
    ...
    const { mutate: onFavorite, isLoading: pendingFavorite } = useApiMutation(
        api.board.favorite
    );

    const { mutate: onUnFavorite, isLoading: pendingUnFavorite } = useApiMutation(
        api.board.unfavorite
    );

    const toggleFavorite = async () => {
        if (isFavorite) {
            await onUnFavorite({ id: id as Id<"boards"> }).catch(() =>
                toast.error("Failed to unfavorite")
            );
        } else {
            await onFavorite({ id: id as Id<"boards">, orgId }).catch(() =>
